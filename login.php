...
// Check the database (if username matches the password)
		...
		if (($data->rowCount() == 1) && ($account_locked == false)) {
			...
			$data = $db->prepare('UPDATE users SET failed_login = "0" WHERE username = (:username) LIMIT 1;');
			$data->bindParam(':username', $username, PDO::PARAM_STR);
			$data->execute();
			...
			$data = $db->prepare('UPDATE users SET failed_login = (failed_login + 1) WHERE username = (:username) LIMIT 1;');
			$data->bindParam(':username', $username, PDO::PARAM_STR);
			$data->execute();
		}

		// Set the last login time
		$data = $db->prepare('UPDATE users SET last_login = now() WHERE username = (:username) LIMIT 1;');
		$data->bindParam(':username', $username, PDO::PARAM_STR);
		$data->execute();
	}
}
